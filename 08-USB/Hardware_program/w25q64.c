...
void SpiFlashInit(void)
{
	...
	// 配置CS引脚为PE15
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;	   // SPI CS (PE15)
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;  // 推挽输出
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; // 推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_Init(GPIOE, &GPIO_InitStructure); // 初始化GPIOE

	// 其他SPI引脚配置保持不变(PA5, PA6, PA7)
	...
	spi_set_speed(SPI_BaudRatePrescaler_8); // 设置速度
}
...
void SpiFlashWriteChar(char tmp, u32 WriteAddr)
{
	...
	uart2_send_str((u8 *)"Byte has been written.\r\n");
}
