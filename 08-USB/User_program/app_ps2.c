...
int16_t combine_bytes(uint8_t byte1, uint8_t byte2) {
    // 小端序：低字节在前，高字节在后
    return (int16_t)((byte2 << 8) | byte1) / 128;
}
...
//		for(int i=0;i<14;i++)
//		{
//			printf("%02x ",ps2_buf[i]);
//		}
//		printf("\r\n");
		
		
		
    if (ps2_buf[4] & 0xff) /* L2 */
        ps2_cmd &= ~0X0001;
    else
        ps2_cmd |= 0X0001;

    if (ps2_buf[5] & 0xff) /* R2 */
        ps2_cmd &= ~0X0002;
    else
        ps2_cmd |= 0X0002;

    if ((ps2_buf[3] & 0x01)) /* L1 */
        ps2_cmd &= ~0X0004;
    else
        ps2_cmd |= 0X0004;

    if ((ps2_buf[3] & 0x02)) /* R1 */
        ps2_cmd &= ~0X0008;
    else
        ps2_cmd |= 0X0008;

    if ((ps2_buf[3] & 0x80)) /* RU */
        ps2_cmd &= ~0X0010;
    else
        ps2_cmd |= 0X0010;

    if ((ps2_buf[3] & 0x20)) /* RR */
        ps2_cmd &= ~0X0020;
    else
        ps2_cmd |= 0X0020;

    if ((ps2_buf[3] & 0x10)) /* RD */
        ps2_cmd &= ~0X0040;
    else
        ps2_cmd |= 0X0040;

    if ((ps2_buf[3] & 0x40)) /* RL */
        ps2_cmd &= ~0X0080;
    else
        ps2_cmd |= 0X0080;

    if ((ps2_buf[2] & 0x20)) /* SE */
        ps2_cmd &= ~0X0100;
    else
        ps2_cmd |= 0X0100;

    if ((ps2_buf[2] & 0x40)) /* AL */
        ps2_cmd &= ~0X0200;
    else
        ps2_cmd |= 0X0200;

    if ((ps2_buf[2] & 0x80)) /* AR */
        ps2_cmd &= ~0X0400;
    else
        ps2_cmd |= 0X0400;

    if ((ps2_buf[2] & 0x10)) /* ST */
        ps2_cmd &= ~0X0800;
    else
        ps2_cmd |= 0X0800;

    if (ps2_buf[2] == 0x01) /* LU */
        ps2_cmd &= ~0X1000;
    else
        ps2_cmd |= 0X1000;

    if (ps2_buf[2] == 0x08) /* LR */
        ps2_cmd &= ~0X2000;
    else
        ps2_cmd |= 0X2000;

    if (ps2_buf[2] == 0x02) /* LD */
        ps2_cmd &= ~0X4000;
    else
        ps2_cmd |= 0X4000;

    if (ps2_buf[2] == 0x04) /* LL */
        ps2_cmd &= ~0X8000;
    else
        ps2_cmd |= 0X8000;
    ...
    */
	...
	int16_t left_x = combine_bytes(ps2_buf[6] , ps2_buf[7]);

	int16_t left_y = combine_bytes(ps2_buf[8] , ps2_buf[9]);

	int16_t right_x = combine_bytes(ps2_buf[10] , ps2_buf[11]);

	int16_t right_y = combine_bytes(ps2_buf[12] , ps2_buf[13]);
